...
const btnConfCita = document.querySelector('.btn_agregar_cita')
...
async function cargarCitas(){
    try{
        ...
        citas = await res.json(); actualizarContadores(); calendario()
...
function calendario(){
    ...
    for(let i = primerDiaMes; i > 0; i--){ liTag += `<li class="inactivo">${ultimoDiaUltimoMes - i + 1}</li>` }
    ...
    if(ultimoDiaMes !== 6){
        for(let i = ultimoDiaMes; i < 6; i++){ liTag += `<li class="inactivo">${i - ultimoDiaMes + 1}</li>` }
    }

    tituloMes.innerHTML = `${meses[mes]} ${anio}`; dias.innerHTML = liTag
}
...
btnConfCita.addEventListener('click', async () => {
    ...
    infoCitas.innerHTML = citasDelDia.map(c => `
        <div class="info_cita" data-id="${c.id_cita}">
            ...
            <div class="motivo_cita">
                <span>SERVICIO:</span>
                <p style="padding-bottom:0.8rem">${c.servicio || 'Sin servicio'}</p>
...
infoCitas.addEventListener('click', async (e) => {
    ...
    if(btnBorrar){
        ...
        } catch(error){ console.error('Error eliminando cita:', error) } return
    }
